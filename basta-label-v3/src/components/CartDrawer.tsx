      ...
      {isOpen && (
        <div className="fixed inset-0 z-[2000] overflow-hidden">
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="absolute inset-0 bg-[#0d0d0b]/40 backdrop-blur-xs transition-opacity"
          />
          ...
          <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
            <motion.div
              ...
              className="w-screen max-w-md bg-[#f0ece3] border-l border-[#ccc9c1]/60 flex flex-col shadow-2xl text-[#0d0d0b] relative"
            >
              {/* Header */}
              <div className="p-6 border-b border-[#ccc9c1]/40 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <ShoppingBag className="w-4 h-4 text-[#888580]" />
                  <h3 className="font-serif text-lg tracking-wide uppercase font-light text-[#0d0d0b]">
                    Mon Panier ({cartItems.reduce((acc, item) => acc + item.quantity, 0)})
                  </h3>
                </div>
                <button
                  onClick={onClose}
                  className="p-1 px-2.5 -mr-2 text-[#888580] hover:text-[#0d0d0b] transition-colors flex items-center gap-1.5"
                >
              ...
              {checkedOutSuccess ? (
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-[#ede9df]/40">
                  ...
                  <span className="text-[0.55rem] font-medium tracking-[0.22em] text-[#888580] uppercase block mb-1">
                    Paiement Réussi · Basta Label
                  </span>
                  <h4 className="font-serif text-2xl font-light text-[#0d0d0b] leading-tight mb-3">
                    Votre commande est réservée.
                  </h4>
                  <p className="text-[0.72rem] text-[#888580] max-w-xs leading-relaxed mb-6">
                  ...
                  <p className="text-[0.65rem] text-[#2b2b27] font-mono mb-8 bg-[#f0ece3] p-3 px-4 border border-[#ccc9c1]/40 rounded-sm">
                  ...
                  <button
                    ...
                    className="w-full max-w-xs py-3.5 bg-[#0d0d0b] text-[#f4f1ea] text-[0.6rem] font-sans font-light tracking-[0.2em] uppercase hover:bg-[#2b2b27] transition-all"
                  >
                    Retourner au Hub
                  </button>
                </div>
                ...
                <>
                  ...
                  <div className="flex-grow overflow-y-auto p-6 focus:outline-none">
                    {cartItems.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-center opacity-80 py-12">
                        ...
                        <p className="font-serif italic text-base text-[#888580] mb-2">
                          Votre panier est silencieux.
                        </p>
                        <p className="text-[0.68rem] text-[#888580] max-w-[240px] leading-relaxed mb-6">
                          Prenez le temps d'explorer la Capsule 01 et de sélectionner des silhouettes faites pour durer.
                        </p>
                        <button
                          onClick={onClose}
                          className="px-6 py-2.5 bg-[#0d0d0b] text-[#f4f1ea] text-[0.55rem] uppercase tracking-widest font-light hover:bg-[#2b2b27] transition-all"
                        >
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="bg-[#ede9df] p-4.5 border border-[#ccc9c1]/35 rounded-sm">
                          <div className="flex items-center gap-2 mb-2 text-[#2b2b27]">
                            <Truck className="w-3.5 h-3.5 text-[#888580]" />
                        ...
                        <div className="divide-y divide-[#ccc9c1]/30">
                          {cartItems.map((item) => (
                            <div key={item.id} className="py-4 first:pt-0 last:pb-0 flex gap-4">
                              ...
                              <div className="w-20 h-26 bg-[#ede9df] border border-[#ccc9c1]/30 overflow-hidden flex-shrink-0 relative">
                                <img
                              ...
                              <div className="flex-1 flex flex-col justify-between">
                                <div>
                                  <div className="flex justify-between items-start">
                                    <h4 className="font-serif text-sm font-medium text-[#0d0d0b]">
                                      {item.product.name}
                                    </h4>
                                    <button
                                      onClick={() => onRemoveItem(item.id)}
                                      className="text-[0.55rem] uppercase tracking-wider text-[#888580] hover:text-[#0d0d0b] transition-colors pl-2"
                                    >
                                      Enlever
                                    </button>
                                  </div>
                                  <p className="text-[0.62rem] text-[#888580] tracking-wide uppercase mt-0.5">
                                ...
                                <div className="flex justify-between items-center mt-3">
                                  {/* Quantity adjusters */}
                                  <div className="flex items-center border border-[#ccc9c1] rounded-sm bg-[#f0ece3] px-1">
                                    <button
                                      onClick={() => onUpdateQuantity(item.id, -1)}
                                      className="p-1 px-1.5 text-[#888580] hover:text-[#0d0d0b] transition-colors"
                                    >
                                  ...
                                  {/* Item Subtotal */}
                                  <span className="text-xs font-sans text-[#2b2b27] tracking-wider">
                  ...
                  {/* Cart Footer Panel */}
                  {cartItems.length > 0 && (
                    <div className="p-6 border-t border-[#ccc9c1]/50 bg-[#ede9df]/50 space-y-4">
                      {/* Subtotal */}
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-xs font-sans text-[#ccc9c1] tracking-wider uppercase font-light">
                          <span>Livraison estimée</span>
                          <span className="text-[#0d0d0b] font-normal">
                            {isFreeShipping ? "Gratuite / Free" : "$15.00 CAD"}
                          </span>
                        </div>
                        <div className="flex justify-between text-base font-serif text-[#0d0d0b] tracking-wide pt-1 border-t border-[#ccc9c1]/20">
                      ...
                      {/* Editorial prompt */}
                      <p className="text-[0.58rem] text-[#888580] leading-relaxed text-center italic">
                      ...
                      <div className="pt-2">
                        <button
                          ...
                          className="w-full py-4 bg-[#0d0d0b] text-[#f4f1ea] text-[0.62rem] font-sans font-light tracking-[0.25em] uppercase hover:bg-[#2b2b27] transition-all disabled:opacity-75 relative flex items-center justify-center overflow-hidden"
                        >
                          {isCheckingOut ? (
                      ...
                      {/* Secure guidelines */}
                      <div className="flex justify-center items-center gap-4 pt-1.5 text-[#888580]">
